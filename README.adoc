...
----
    <properties>
  	    <docker.image.prefix>fortunecookiezen</docker.image.prefix>
    </properties>
    <build>
        <plugins>
...
* The image name (or tag), which will end up here as `fortunecookiezen/gs-spring-boot-docker`
...
$ docker push fortunecookiezen/gs-spring-boot-docker
----

NOTE: You don't have to push your newly minted Docker image to actually run it. Moreover the "push" command will fail if you aren't a member of the "fortunecookiezen" organization on Dockerhub. Change the build configuration and the command line to your own username instead of "fortunecookiezen" to make it actually work.
...
* the image name (or tag) is set up from the jar file properties, which will end up here as `fortunecookiezen/gs-spring-boot-docker`
* the directory in which to find the Dockerfile
...
The "docker push" will fail for you (unless you are part of the "fortunecookiezen" organization at Dockerhub), but if you change the configuration to match your own docker ID then it should succeed, and you will have a new tagged, deployed image.
...
The application is then available on http://localhost:8080 (visit that and it says "Hello Docker World"). To make sure the whole process is really working, change the prefix from "fortunecookiezen" to something else (e.g. `${env.USER}`) and go through it again from the build through to the docker run. 
...
81c723d22865        fortunecookiezen/gs-spring-boot-docker:latest   "java -jar /app.jar"   34 seconds ago      Up 33 seconds       0.0.0.0:8080->8080/tcp   goofy_brown         
----
...
----
$ docker run -e "SPRING_PROFILES_ACTIVE=production" -p 8443:8443 -t fortunecookiezen/gs-spring-boot-docker
----

or

----
$ docker run -e "SPRING_PROFILES_ACTIVE=development" -p 8443:8443 -t fortunecookiezen/gs-spring-boot-docker
----

To make the test cases work, because we have introduced ssl support, you will need to import the server's public key into your local jre cacert file

----
sudo keytool -import -file gs-spring-boot-docker-demo-certificate.cer -alias docker-demo-cert -keystore $JAVA_HOME/lib/security/cacerts
----
